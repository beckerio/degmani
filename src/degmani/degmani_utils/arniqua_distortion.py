...
GROUP_MARKERS = {
    "brightness_change": "o",   # circle
    "blur": "s",                # square
    "color_distortion": "^",    # triangle up
    "jpeg": "D",                # diamond
    "noise": "P",               # plus (filled)
    "spatial_distortion": "X",  # X (filled)
    "sharpness_contrast": "v",  # triangle down
...
# Per-group color schemes (ColorBrewer sequential palettes)
#Terracotta: ["#f3d7c7", "#d48a6a", "#9c3f2f"]
# Blues: ["#c6dbef", "#6baed6", "#2171b5"]
#Slate: ["#dfe2e6", "#8a9aaf", "#4a5d73"]
Gold = ["#fff3b0", "#f9c74f", "#c28f2c"]
group_palettes = {
    "brightness_change": Gold, #["#c6dbef", "#6baed6", "#2171b5"],                           #
    "blur": ["#d4b9da", "#9e9ac8", "#6a51a3"],                                        # Purples
    "color_distortion": ["#c7e9c0", "#74c476", "#31a354", "#006d2c"],                  # Greens
    "jpeg": ["#bdbdbd", "#636363"],                                                   # Greys
    "noise": ["#fdd0a2", "#fdae6b", "#fd8d3c", "#d94801"],                             # Oranges
    "spatial_distortion": ["#fde0dd", "#fa9fb5", "#f768a1", "#dd3497", "#ae017e"],     # RdPu
    "sharpness_contrast": ["#fcbba1", "#fb6a4a", "#cb181d"],                           # Reds
}

# Build the mapping (name + per-group color)

PRISTINE_KEY = "Pristine"
PRISTINE_GROUP = "Pristine"
PRISTINE_COLOR = "#17BECF"  # vivid cyan (colorblind-friendly)
PROTO_COLOR = "pink"  #

def get_nice_mapping_distortion()-> Dict[str, Tuple[str, str]]:
    nice_mapping_distortion = {}
    for group, keys in distortion_groups.items():
        palette = group_palettes[group]
        if len(palette) != len(keys):
            raise ValueError(f"Palette length mismatch for group '{group}': "
                             f"{len(palette)} colors for {len(keys)} items.")
        for key, color in zip(keys, palette):
            nice_mapping_distortion[key] = (full_names[key], color)

    nice_mapping_distortion["Pristine"] = ("Pristine", PRISTINE_COLOR)
    nice_mapping_distortion["Prototype"] = ("Prototype", PROTO_COLOR)
    return nice_mapping_distortion
...
def distort_images(image: torch.Tensor, distort_functions: list = None, distort_values: list = None,
    ...
    image = image.clone()       # Prevents changes to original image
