"""
https://github.com/glhr/beyond-AUROC/tree/main
...
def plot_ood_scores(id_data, ood_data, model_name=None, plot=True, save=False, clip=False, plot_labels=True,
                    plot_threshs=True, return_metrics=False, plot_tight=True):
    if clip:
    ...
    if plot:
        count, bins, ignored = plt.hist([id_data, ood_data], 20, density=True, color=["cornflowerblue", "violet"],
                                        label=["ID", "OOD"], alpha=0.7, histtype='step', fill=True)
        ...
        if plot_threshs:
            for thresh_color, thresh_label in zip(["purple", "green"], ["thresh_95tpr", "thresh_95tnr"]):
                plt.axvline(x=c["metrics"][thresh_label], label=f"@{thresh_label.replace('thresh_', '')}",
                            color=thresh_color, linestyle="--")
        if plot_labels:
            ...
            plt.legend([handles[idx] for idx in order], [labels[idx] for idx in order],
                       bbox_to_anchor=(0, 1.02, 1, 0.2), loc="lower center", mode="expand", borderaxespad=0,
                       ncol=4 if plot_threshs else 2)
